...
const server = createServer(async (request, response) => {
  try {
    ...
    if (!target.startsWith(`${build}${path.sep}`)) throw new Error();
...
const errors = [];
const heicBytes = await readFile('/tmp/heic-spike/example.heic');
...
try {
  ...
  const decoder=[]; page.on('request', r=>{if(r.url().includes('heic-to-1.5.2')) decoder.push(r.url())});
  await page.goto(`${origin}/heic-converter/`);
  const webp=await makeWebp(page);
  await page.locator('#files').setInputFiles(webp);
  ...
  await page.locator('#files').setInputFiles(heicFile); await page.waitForFunction(()=>!document.querySelector('#convert').disabled); await page.selectOption('#format','jpeg'); await page.locator('#convert').click(); await page.waitForSelector('.result',{timeout:120000});
  out=await inspectResult(page);assert.equal(out.type,'image/jpeg');assert.ok(out.width>0&&out.height>0&&out.size>0&&out.previewComplete);assert.ok(out.pixel[3]===255);assert.ok(decoder.length>0);assert.match(await page.locator('#readiness').textContent(),/曾成功加载/);
  await page.selectOption('#format','png');await page.locator('#convert').click();await page.waitForFunction(()=>document.querySelector('.result a')?.download.endsWith('.png'),null,{timeout:120000});out=await inspectResult(page);assert.equal(out.type,'image/png');assert.ok(out.size>0&&out.width>0);

  const webp2=await makeWebp(page,'mixed.webp','#205bd7');await page.locator('#files').setInputFiles([heicFile,webp2]);await page.waitForFunction(()=>!document.querySelector('#convert').disabled);await page.locator('#convert').click();await page.waitForFunction(()=>document.querySelectorAll('.result').length===2,null,{timeout:120000});assert.equal(await page.locator('.result').count(),2);const mixed2=await inspectResult(page,1);assert.deepEqual([mixed2.width,mixed2.height],[31,19]);assert.ok(mixed2.pixel[2]>150);
  ...
  const licenseStatus=await page.evaluate(async()=>({license:(await fetch('./third-party/heic-to-1.5.2.LICENSE.txt')).status,source:(await fetch('./third-party/heic-to-1.5.2-source/index.js')).status,library:(await fetch('./third-party/heic-to-1.5.2.csp.js')).status}));assert.deepEqual(licenseStatus,{license:200,source:200,library:200});
  ...
  console.log(JSON.stringify({webp:{dimensions:[31,19],decoderRequests:0},heic:{dimensions:[out.width,out.height],decoderRequests:decoder.length},mixed:2,staleInvalidated:true,mobile:layout,errors},null,2));
